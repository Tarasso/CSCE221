...
public:
  ...
  // Constuctor, takes given heap and percolates it to become min heap
  MinHeap(Heap<T, m_size>& origHeap)
  {
    this->m_array = origHeap.getM_Array();
    this->numPins = origHeap.getNumPins();
    this->currentMaxSize = origHeap.getCurrentMaxSize();

    //Sorting heap in min order
  ...
  // until min heap conditions are met
  ...
  // PercolateDown continously swaps parent with smallest child
  // until min heap conditions are met
